...
export function getYACCMode(
	yaccLanguageService: YACCLanguageService,
): LanguageMode {
	const cache = CreateDocumentCache<YACCDocument>(10, 60, (document) =>
		yaccLanguageService.parseYACCDocument(document),
	);
	return {
		getId() {
			return 'yacc';
		...
		doValidation(
			...
			if (force) {
				return yaccLanguageService.doValidation(
					document,
					yaccLanguageService.parseYACCDocument(
						document,
					),
				);
			}
		...
		doComplete(
			...
			const yacc = cache.get(document);
			return yaccLanguageService.doComplete(
				document,
				position,
				yacc,
			);
		...
		doHover(
			...
			const yacc = cache.get(document);
			return yaccLanguageService.doHover(
				document,
				position,
				yacc,
			);
		...
		findTypeDefinition(
			...
			const yacc = cache.get(document);
			return yaccLanguageService.findTypeDefinition(
				document,
				position,
				yacc,
			);
		...
		findDefinition(
			...
			const yacc = cache.get(document);
			return yaccLanguageService.findDefinition(
				document,
				position,
				yacc,
			);
		...
		findReferences(
			...
			const yacc = cache.get(document);
			return yaccLanguageService.findReferences(
				document,
				position,
				yacc,
			);
		...
		doRename(
			...
			const yacc = cache.get(document);
			return yaccLanguageService.doRename(
				document,
				position,
				newName,
				yacc,
		...
		getSemanticTokens(document: TextDocument): SemanticTokenData[] {
			const yacc = cache.get(document);
			return yaccLanguageService.getSemanticTokens(
				document,
				yacc,
			);
		},
