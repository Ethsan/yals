				...
				switch (ch) {
					...
					case _LAN: // <
						if (
							...
							// <
							state =
								ScannerState.WithinPredefined;
							return finishToken(
								offset,
								TokenType.StartPredefined,
							);
						}
						state =
							ScannerState.WithinStates;
						return finishToken(
							offset,
							TokenType.StartStates,
						);
					case _BAR: // |
						return finishToken(
							offset,
					...
					case _BOP: // {
						state =
							ScannerState.WithinAction;
						return finishToken(
							offset,
							TokenType.StartAction,
						);
					case _WSP: // ' '
				...
				while (!exit && brackets > 0) {
					...
					switch (ch) {
						...
						case _DQO: // "
							stream.goBack(1);
							if (!nextLiteral())
								// skip string if not skip character
								stream.advance(
									1,
								);
							break;
