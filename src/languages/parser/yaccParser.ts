...
export function parse(text: string): YACCDocument {
	...
	function addSymbolToMap(
		...
		if (old) {
			addProblem(
				`Symbol was already declared/defined.`,
				offset,
				end,
				ProblemType.Error,
				{
	...
	function addUnknownSymbolProblem(scanner: Scanner) {
		addProblem(
			`Unknown symbol ${scanner.getTokenText()}.`,
		...
		switch (token) {
			...
			case TokenType.Option:
				...
				switch (tokenText) {
					...
					case '%precedence':
						lastNode = {
							nodeType: NodeType.Precedence,
							offset: offset,
							length: -1,
							end: -1,
						};
						state =
							ParserState.WaitingPrecedence;
						break;
					default:
						break;
				}
			...
			case TokenType.TypeValue: {
				...
				if (t) {
					...
					addProblem(
						`Type was not declared in the %union.`,
						scanner.getTokenOffset(),
						scanner.getTokenEnd(),
						ProblemType.Error,
					);
			...
			case TokenType.Word: {
				...
				switch (state) {
					...
					case ParserState.WaitingToken:
						if (predefined[word]) {
							addProblem(
								`You cannot declare the preserved keyword "${word}" as a token!`,
								offset,
								scanner.getTokenEnd(),
								ProblemType.Error,
							);
				...
				switch (state) {
					case ParserState.WaitingRule: {
						...
						if (
							...
							addProblem(
								`Unexpected ':' you can only declare a non-terminal with a word.`,
								scanner.getTokenOffset(),
								scanner.getTokenEnd(),
								ProblemType.Error,
							);
						...
						const nonTerminal =
							document.components.pop(); // the last symbol was not part of last rule
						if (nonTerminal !== undefined) {
							// I think the array will never be empty, but check for sanity
							if (
								lastNode !==
								undefined
							) {
							...
							if (
								...
								addProblem(
									`You cannot declare the preserved keyword "${nonTerminal.name}" as a non-terminal!`,
									nonTerminal.offset,
									nonTerminal.end,
									ProblemType.Error,
								);
							...
							nonTerminal.terminal =
								false; // this will not be a terminal
							...
							if (
								...
								// if the symbol was previously declared with %type ...
								if (
									!symbol.terminal
								) {
									// there is a redefinition of the symbol
									addProblem(
										`Non-terminal symbol was already declared.`,
										nonTerminal.offset,
										nonTerminal.end,
										ProblemType.Error,
										{
								...
								nonTerminal.references.push(
									symbol
										.references[0],
								); // add %type reference
								nonTerminal.type =
									symbol.type; // assign the type from %type
								symbol.references =
									nonTerminal.references; // update also the old references
							}
							const token =
								document.tokens[
									nonTerminal
							...
							if (
								...
								// if the symbol was already declared as a token
								addProblem(
									`Symbol was already declared as a token.`,
									nonTerminal.offset,
									nonTerminal.end,
									ProblemType.Error,
									{
							...
							document.symbols[
								nonTerminal.name
							] = nonTerminal; // update symbol table
							lastNode = {
								nodeType: NodeType.Rule,
								name: nonTerminal.name,
								offset: nonTerminal.offset,
								length: -1,
								end: -1,
								actions: [],
							};
			...
			case TokenType.Param:
				if (state == ParserState.WaitingRule) {
					...
					switch (lastToken) {
						...
						default:
							addProblem(
								`Named reference to either non terminal or middle rule action`,
								scanner.getTokenOffset(),
								scanner.getTokenEnd(),
								ProblemType.Error,
							);
						...
						if (
							...
							lastTokenSymbol.alias =
								addSymbolToMap(
									...
									scanner.getTokenEnd(),
									word,
									lastTokenSymbol.type,
								);
							if (
								lastTokenSymbol.alias
							)
								lastTokenSymbol.alias.alias =
									lastTokenSymbol;
						} else {
							addProblem(
								`Alias not associated with an token.`,
								scanner.getTokenOffset(),
								scanner.getTokenEnd(),
								ProblemType.Error,
							);
	...
	for (let i = 0; i < document.components.length; i++) {
		...
		if ((symbol = document.symbols[component.name])) {
			...
			symbol.used = true;
			if (symbol.alias) symbol.alias.used = true;
	...
	Object.keys(document.symbols).forEach((key) => {
		if (
			document.symbols[key].definition[0] <
			document.rulesRange[0]
		) {
			addProblem(
				'Non-terminal symbol type defined but never declared.',
